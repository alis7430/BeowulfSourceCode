...
public class Quest
{
    ...
    public void GetReward()
    {
        ...
        if (itemReward != null)
        {
            GameObject itemPickedUp = GameObject.Instantiate(itemReward.gameObject);
            Item item = itemPickedUp.GetComponent<Item>();
            //Debug.Log(itemPickedUp.name);
            if (!item.pickedUp)
            {
                GameObject.FindGameObjectWithTag("Player").GetComponent<Inventory>().AddItem(itemPickedUp, item);
            }
        }
