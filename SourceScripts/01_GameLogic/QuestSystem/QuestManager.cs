...
public class QuestManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        GameObject questListContent = UIManager.Instance.questListContent;
        ...
        EventManager.Instance.AddListener(EVENT_TYPE.ACTIVE_PORTAL, OnEvent);
    }
    ...
    private void CheckQuestStartEvent(int questId)
    {
        switch (questId)
        {
            ...
            case 104:
                currentQuest[questId].questGiver.NextDialogue();
                GameObject tutorialPortal = GameObject.Find("TutorialPortal");
                tutorialPortal.transform.GetChild(0).gameObject.SetActive(true);
                break;
            case 201:
                currentQuest[questId].questGiver.NextDialogue();
                break;
            case 202:
                GameObject eg = GameObject.FindGameObjectWithTag("EventObj");
                eg.transform.GetChild(0).gameObject.SetActive(true);
                currentQuest[questId].questGiver.NextDialogue();
                break;
            case 203:
                currentQuest[questId].questGiver.NextDialogue();
    ...
    private void CheckQuestCompleteEvent(int questId)
    {
        switch (questId)
        {
            ...
            case 202:
                currentQuest[questId].questGiver.NextDialogue();
                GameObject eg = GameObject.FindGameObjectWithTag("EventObj");
                eg.transform.GetChild(1).gameObject.SetActive(true);
                break;
            case 203:
                currentQuest[questId].questGiver.NextDialogue();
                break;
    ...
    public void CompleteQuest(Quest quest)
    {
        ...
        if(quest.questGiver.HasQuest())
            quest.questGiver.SetState(NPC_STATE.CLEARQUEST);
        else
            quest.questGiver.SetState(NPC_STATE.IDLE);
    ...
    private void CheckPortalID(int ID)
    {
        List<Quest> qList = new List<Quest>(currentQuest.Values);
        foreach (Quest quest in qList)             //수행중인 퀘스트 순회
        {
            if (quest != null)
            {
                if (quest.goal.goalType == QuestGoalType.PORTAL) //적을 죽이는 퀘스트일때
                {
                    if (quest.goal.portal.ID == ID)           //죽인 적이 맞는지 확인
                    {
                        CompleteQuest(quest);                    //퀘스트 완료
                    }
                }
    ...
    protected virtual void OnEvent(EVENT_TYPE Event_Type, Component Sender, object Param = null)
    {
        switch (Event_Type)
        {
            ...
            case EVENT_TYPE.ENEMY_KILLED:
                ...
                break;
            case EVENT_TYPE.ACTIVE_PORTAL:
                CheckPortalID((int)Param);
                break;
            default:
                break;
        }
