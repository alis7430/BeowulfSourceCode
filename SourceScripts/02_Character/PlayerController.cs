...
public class PlayerController : BaseCharacter
{
    ...
    protected override void Initialize()
    {
        ...
        playerSkill = this.transform.GetComponent<PlayerSkill>();
    ...
    private void Update()
    {
        ...
        {
            this.Heal(HealthRecovery);
            this.StaminaHeal(StaminaRecovery);
        ...
        if (this.next_step == STEP.NONE)
        {
            switch (this.step)
            {
                case STEP.IDLE:
                    ...
                    //스킬 1 사용
                    if (Input.GetKeyDown(KeyCode.Alpha1) && STAMINA > 15)
                    {
                        if (is_Player_Armed)
                        {
                            next_step = STEP.SKILL1;
                            STAMINA -= 15;
                        }
                    }
                    //스킬 2 사용
                    if (Input.GetKeyDown(KeyCode.Alpha2) && STAMINA > 30)
                    {
                        if (is_Player_Armed)
                        {
                            next_step = STEP.SKILL2;
                            STAMINA -= 30;
                        }

                    }
                    //스킬 3 사용
                    if (Input.GetKeyDown(KeyCode.Alpha3) && STAMINA > 25)
                    {
                        if (is_Player_Armed)
                        {
                            next_step = STEP.SKILL3;
                            STAMINA -= 25;
                        }

                    }
                    //스킬 4 사용
                    if (Input.GetKeyDown(KeyCode.Alpha4) && STAMINA > 25)
                    {
                        next_step = STEP.SKILL4;
                        STAMINA -= 25;
                    }
                    break;
                ...
                case STEP.MOVE:
                    ...
                    if (Input.GetKeyDown(KeyCode.LeftControl) && (Input.GetButton("Horizontal") || Input.GetButton("Vertical")))
                    {
                        next_step = STEP.ROLL;
                    }
                    //스킬 1 사용
                ...
                case STEP.ATTACK2:
                    ...
                    break;
                case STEP.ATTACK3:
                    if (stepTimer > 1.3f * attackSpeedRatio)
                    {
                        next_step = STEP.IDLE;
                    }
                    if (Input.GetKeyDown(KeyCode.LeftControl) && (Input.GetButton("Horizontal") || Input.GetButton("Vertical")))
                    ...
                    break;
                case STEP.ROLL:
                    if (stepTimer > 1.7f * movingSpeedRatio)
                    {
        ...
        while (this.next_step != STEP.NONE)
        {
            ...
            switch (this.step)
            {
                ...
                case STEP.ROLL:
                    ...
                    tempMove = new Vector3(Input.GetAxisRaw("Horizontal"), 0, Input.GetAxisRaw("Vertical"));

                    break;
                case STEP.SKILL1:
                    ani.Play("Standing Melee Skill1");
                    stopMove();
                    Invoke("ActiveSkill1", 0.6f * attackSpeedRatio);
                    is_Calculate_Move = false;
                    break;
                case STEP.SKILL2:
                    ani.Play("Standing Melee Skill2");
                    Invoke("ActiveSkill2", 1.1f * attackSpeedRatio);
                    SoundManager.instance.PlaySFX("Skill2Sound", 0.7f * attackSpeedRatio);

                    stopMove();
                    is_Calculate_Move = false;
                    break;
                case STEP.SKILL3:
                    ani.Play("Standing Melee Skill3");
                    Invoke("ActiveSkill3", 0.8f * attackSpeedRatio);
                    SoundManager.instance.PlaySFX("Skill3Sound");

                    stopMove();
                    is_Calculate_Move = false;
                    break;
                case STEP.SKILL4:
                    ani.Play("Standing Melee Skill4");
                    Invoke("ActiveSkill4", 0.6f * attackSpeedRatio);
                    SoundManager.instance.PlaySFX("BattleCry");

                    stopMove();
                    is_Calculate_Move = false;
                    break;
                case STEP.DEAD:
                    is_Gradient_Check = false;       // 기본 상태에서는 경사로 검색을 해줌
                    is_Calculate_Move = false;       // 기본 상태에서는 이동이 가능
                    is_CameraMoveOnPlayer = false;
        ...
        switch (this.step)
        {
            ...
            case STEP.WEAPON_CHANGE:
                ...
                break;
            case STEP.ATTACK1:
                if (stepTimer > 0.6f * attackSpeedRatio && weaponCollider.enabled == false)
                {
                    weaponCollider.enabled = true;
                }
                break;
            case STEP.ATTACK2:
                if (stepTimer > 0.3f * attackSpeedRatio && weaponCollider.enabled == false)
                {
                    weaponCollider.enabled = true;
                }
                break;
    ...
    void LateUpdate()
    {
        if (is_CameraMoveOnPlayer && is_player_Control)
        {
    ...
    protected override void OnAttacked(object param)
    {
        ...
        if (HEALTH <= 0 && !is_dead)
        {
            EventManager.Instance.PostNotification(EVENT_TYPE.DEAD, this, null);
            SoundManager.instance.PlayBGM("Dead");
            is_dead = true;
        }
    }
    ...
    //-----------------------------------------------------------
    public void MoveToPoition(Vector3 traget)
    {
    }
    // ----------------------------------------------------------
    // Private Fuctions
    ...
    void CalculateMove(float increaseRatio)
    {
        ...
        if (Input.GetButton("Horizontal") || Input.GetButton("Vertical"))
        {
            ...
            transform.rotation = Quaternion.Lerp(transform.rotation, cameraRotation, 30.0f * Time.deltaTime);      //자연스러움을 위해 Slerp로 회전시킨다.
            ...
            if (move != Vector3.zero)       //Quaternion.LookRotation는 (0,0,0)이 들어가면 경고를 내므로 예외처리 해준다.
            {
                ...
                modelTransform.rotation = Quaternion.Lerp(modelTransform.rotation, characterRotation, 20.0f * Time.deltaTime);
            }
    ...
    //-----------------------------------------------------------
    //구르기 사용시 호출
    void Roll(Vector3 inputDirection)
    {
        if (!is_Calculate_Move)
            return;
        ...
        Vector3 inputMoveXZ = inputDirection;
        ...
        Vector3 moveXZ = inputMoveXZ * 1.5f;
        move = Vector3.MoveTowards(move, moveXZ, runSpeed * 2.0f);
    ...
    {
        playerSkill.ActiveSkill(1);
    }
    //-----------------------------------------------------------
    private void ActiveSkill2()
    {
        playerSkill.ActiveSkill(2);
    }
    //-----------------------------------------------------------
    private void ActiveSkill3()
    {
        playerSkill.ActiveSkill(3);
    }
    //-----------------------------------------------------------
    private void ActiveSkill4()
    {
        playerSkill.ActiveSkill(4);
    }
    #endregion
}
