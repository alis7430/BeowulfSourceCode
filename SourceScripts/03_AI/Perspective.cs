...
public class Perspective : Sense
{
    ...
    void DetectAspect()
    {
        ...
        rayDirection = playerTransform.position - transform.position + offset;

        //인공지능 캐릭터의 전방 벡터 그리고 플레이어와 인공지능 간의 방향 벡터 사이의 각도를 검사한다.
        if((Vector3.Angle(rayDirection, transform.forward)) < FieldOfView)
        {
            //플레이어가 필드 내 보이는 영역에 있는지 검사한다.
            if (Physics.Raycast(transform.position, rayDirection.normalized, out hit,ViewDistance, targetMask))
            {
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 frontRayPoint = transform.position + offset + (transform.forward * ViewDistance);

        //대략적인 시야 시각화
        Vector3 leftRayPoint = transform.position + offset + TransDegreeToZAxis(-FieldOfView) * ViewDistance;
        Vector3 rightRayPoint = transform.position + offset + TransDegreeToZAxis(FieldOfView) * ViewDistance;
